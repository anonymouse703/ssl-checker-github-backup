...
function releaseDomainLock(domain, pid) {
  ...
  try {
    ...
    if (owner === String(pid)) fs.unlinkSync(lockFile);
  } catch (_) {}
}
...
function handleRequest(req, res) {
  ...
  if (method === "POST" && url.pathname === "/scan") {
    return parseBodyJson(req, (err, parsed) => {
      ...
      // ✅ REMOVED: the hard 429 cap block that was here.
      // The browser pool queue now handles backpressure — users
      // wait for a slot instead of getting rejected outright.

      // ✅ KEEP: duplicate domain guard (unchanged)
      ...
      // ✅ KEEP: cross-process domain lock guard (unchanged)
      ...
      const child = spawn("node", ["index.js", domain], {
        ...
        env: {
          ...
          // ✅ ADDED: tells index.js to use the shared pool
          // instead of calling launchBrowser() directly
          BROWSER_POOL_SKIP: "1",
        },
      });

      // Update lock file with actual child PID (unchanged)
      try {
      ...
      child.stdout.on("data", (data) =>
        appendTextSafe(logFilePath(jobId), String(data)),
      );
      child.stderr.on("data", (data) =>
        appendTextSafe(logFilePath(jobId), "[stderr] " + String(data)),
      );
      ...
      return jsonResponse(res, 200, {
        ...
        // ✅ ADDED: FileMaker can read pool_stats.queued to show
        // "your scan is #3 in the queue" instead of just "started"
        pool_stats: poolStats(),
      });
    });
  }
  ...
  if (method === "GET" && url.pathname === "/status") {
    ...
    // Try to identify the job by the domain currently in activeJobs,
    // or fall back to scanning the OUTPUT_DIR for the most recent progress file
    // for this domain (useful for jobs started before the server restarted).
    let jobId = null;
    let progress = null;
    let isDone = false;
      ...
      try {
        const files = fs
          ...
          .filter(
            (f) =>
              f.startsWith(
                `progress_${domain.replace(/[^a-z0-9._-]/gi, "_")}`,
              ) && f.endsWith(".txt"),
          )
          .map((f) => ({
            f,
    ...
    const statusText = isDone
      ? "Audit complete"
      : progress?.status || (isRunning ? "Running scan..." : "Idle");
